...
**Ask more during implement.** Present bounded **implementation choices**, mix MCQ/open, and have the student **complete snippets in real code files**. Re-score implement confidence after each check; **as confidence drops, increase** remaining questions and snippets (see student-build implement confidence ladder). Do not dump a finished workflow while they only watch. **MCQ options must not give away the answer**. Log `<!-- student-build:code-check … -->` with `implement_confidence`. Do not turn the phase into an exam. Do not quiz starter-kit auth internals. A mid/low-confidence workflow with zero student file snippets is an anti-pattern.
...
- Implementing every layer yourself with no student file snippets when implement confidence is below 0.90
