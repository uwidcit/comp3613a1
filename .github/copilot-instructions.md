...
- When they ask to judge or grade the chat, **or** to build/export the report: use skill **student-judge** and do not keep coding. Write the scorecard to `docs/judge.md`. Score COMP 3613 phases 1–5. Read all native agent chats for this project. Students never put transcripts in the repo. Do not penalize missing explore reports or starter-kit auth lectures. Show awarded total / scoreable max, then overall / 4. Then export if they asked (`python manage.py report`).
