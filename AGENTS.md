...
- Phase 2: draft the use-case diagram from the `Feature (user)` lines. Assume actors and grouping. Do not withhold the draft. Choice chips are OK when options are real and bounded; do not invent a pick-list.
...
- Phase 5: ask for theming and branding preferences, then implement one workflow at a time in the same chat. After they verify, continue here. Do not require a new chat per workflow.
...
- Co-draft `docs/report.md` from their decisions. They may export an incomplete PDF at any phase: `python manage.py report --name "..." --id "..."`. Do not refuse a draft export. Export hashes the course skills and fails if they were edited. Do not edit `.agents/skills/`, `.cursor/skills/`, `AGENTS.md`, or `.agents/skills.lock.json`.
